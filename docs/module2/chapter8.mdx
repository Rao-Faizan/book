...
- 🎨 **Computer vision training**: Need photorealistic images
- 👥 **Human-robot interaction**: Realistic human characters and animations
- 📊 **Synthetic data generation**: Large-scale annotated datasets
- 🎬 **Demonstrations/Marketing**: Beautiful renders for presentations
:::
...
Should see: `data: 'Hello from Unity!'` 🎉
...
**Create New Scene:**
```
File → New Scene → Basic (Built-In) → Save as "RobotWarehouse"
```
...
**Configure:**
- Scale: `(10, 1, 10)` → 100m² floor
- Position: `(0, 0, 0)`

**Add Material:**
1. `Assets → Create → Material` → Name: `FloorMaterial`
...
**In Volume Profile:**
- ✅ Enable `Ambient Occlusion` (realistic shadows in corners)
- ✅ Enable `Screen Space Reflections` (mirror-like surfaces)
- ✅ Enable `Indirect Lighting Controller`
...
```
GameObject → Light → Point Light (x4, in corners)
```
...
:::warning Unity ↔ ROS Coordinate Systems
...
public class JointCommandSubscriber : MonoBehaviour
{
    ...
    void ApplyJointCommands(JointStateMsg msg)
    {
        for (int i = 0; i < msg.position.Length && i < joints.Length; i++)
        {
            ...
            drive.target = (float)msg.position[i] * Mathf.Rad2Deg;  // Rad → Deg
...
2. `Inspector → Add Component → Labeling`
...
│   ├── Walls (x4 Cubes)
│   ├── Furniture (Tables, Chairs from Asset Store)
│   └── Lighting (Directional + 4 Point Lights)
├── Robot
│   ├── Humanoid (Imported URDF)
│   ├── Camera (with Perception)
│   └── Sensors (Simulated LiDAR - next chapter)
├── ROS Integration
│   ├── ROSConnectionManager (Empty GameObject + Script)
│   ├── VelocitySubscriber (on Robot)
│   └── PosePublisher (on Robot)
└── UI
    └── Canvas (Debug info display)
```
...
public class NavigationIntegration : MonoBehaviour
{
    ...
    PointMsg ToROSPosition(Vector3 unityPos)
    {
        ...
        {
            x = unityPos.x,
            y = unityPos.z,  // Unity Y → ROS Z
            z = unityPos.y   // Unity Z → ROS Y
        };
    }
...
- **Coordinate system conversion** is critical (Unity Y-up ↔ ROS Z-up)

### 📚 Unity vs Gazebo Summary
...
- 📘 [Unity Robotics Hub](https://github.com/Unity-Technologies/Unity-Robotics-Hub) - Complete tutorials
- 🔌 [ROS-TCP-Connector Docs](https://github.com/Unity-Technologies/ROS-TCP-Connector) - API reference
- 🎨 [HDRP Documentation](https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@latest) - Lighting guide

### Video Tutorials
- 🎥 [Unity Robotics Tutorial Series](https://www.youtube.com/playlist?list=PLQMQNmwN3FvzVl4Fg8zFlpPpZVVWYvfz-) - Official Unity
- 🎥 [URDF Import Walkthrough](https://www.youtube.com/watch?v=<example>) - Step-by-step

### Community
- 💬 [Unity Robotics Discord](https://discord.gg/unity-robotics) - Active community
- 💬 [ROS Discourse - Unity](https://discourse.ros.org/tag/unity) - Integration questions
...
Get ready to give your robots **eyes and ears**! 👀🦻

---

:::note Chapter Completion
✅ You've completed Chapter 8: Unity for High-Fidelity Rendering  
⏱️ Estimated time to complete: 90 minutes  
📊 Progress: Module 2 - Chapter 3 of 5
:::
