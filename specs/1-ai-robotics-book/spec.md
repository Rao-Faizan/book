...
- Q: Should Module 1's content or exercises include any considerations or notes regarding accessibility or localization for the student-facing materials, given the global nature of robotics education? → A: Incorporate basic accessibility guidelines (e.g., image alt text, clear headings, color contrast) and ensure English is primary.

- Q: For performance, what are the target p95 latency and throughput requirements for ROS 2 node communication within Module 1's exercises, especially for critical control loops in Chapter 3? → A: p95 latency < 100ms, throughput > 100 messages/sec for control loops.

- Q: What specific data retention policies or guidelines should be communicated to students regarding any data generated by their ROS 2 applications during exercises (e.g., log files, sensor data recordings)? → A: Advise students to manage their own data retention; provide optional best practices for temporary storage and deletion.

- Q: What is the policy regarding potential deviations or customizations to the core ROS 2 Humble distribution, such as using alternative DDS implementations or specific third-party ROS 2 packages? → A: Strict adherence to ROS 2 Humble; no alternative DDS or third-party packages allowed in Module 1 to maintain consistency and simplify support.

- Q: How should the course handle potential inconsistencies or breaking changes in ROS 2 APIs or recommended practices that might occur between the development of the module and its delivery? → A: Standardize on a fixed ROS 2 Humble version for the duration of the module and provide notes on known limitations or workarounds for any critical breaking changes.
...
2. **Given** a student completes the nodes/topics/services section, **When** presented with a robotics use case (e.g., sensor data collection), **Then** they can determine whether to use topics (continuous data streams) or services (request-response patterns) or actions
3. **Given** a student studies the computational graph examples, **When** they examine a multi-node system diagram, **Then** they can identify publishers, subscribers, service clients, service servers, action clients, and action servers
...
- **FR-022**: Chapter MUST introduce standard message types (std_msgs, geometry_msgs, sensor_msgs, rcl_interfaces) relevant to robotics
- **FR-023**: Chapter MUST teach parameter declaration and access using declare_parameter() and get_parameter() methods
- **FR-024**: Chapter MUST explain QoS profiles in practice (QoSProfile class) and common presets (sensor_data, system_default, services_default)
...
- **FR-026**: Chapter MUST include debugging techniques using ROS 2 CLI (ros2 node list, ros2 topic echo, ros2 service call, ros2 param get) and recommended VS Code debugging features (breakpoints, variable inspection) with note that alternative IDE users should adapt instructions
...
- **FR-043**: Chapter MUST explain visual geometry using primitive shapes (box, cylinder, sphere) and mesh files (STL, DAE, OBJ)
- **FR-044**: Chapter MUST explain collision geometry for physics simulation (simplified vs. visual geometry)
- **FR-045**: Chapter MUST explain inertial properties (mass, center of mass, inertia tensor) and their importance for realistic simulation
- **FR-046**: Chapter MUST teach joint properties (axis, origin, limits, dynamics, calibration) and their influence on robot motion
- **FR-047**: Chapter MUST demonstrate building a simple multi-link robot URDF (e.g., 2-DOF arm, mobile base)
- **FR-048**: Chapter MUST demonstrate building humanoid-specific URDF with torso, head, arms, legs, hands, and feet (e.g., using `xacro`)
- **FR-049**: Chapter MUST explain kinematic chains and joint hierarchies for humanoid robots, including concepts like forward and inverse kinematics
- **FR-050**: Chapter MUST teach URDF validation using check_urdf tool and common error debugging techniques
- **FR-051**: Chapter MUST show visualization in RViz2 with robot_state_publisher and joint_state_publisher nodes
- **FR-052**: Chapter MUST explain xacro macros for parameterized and reusable URDF components and how to use them effectively
- **FR-053**: Chapter MUST provide reference frames and naming conventions for humanoid robots (base_link, left_hand, right_foot, etc.) following REP-103 and REP-105 standards
- **FR-054**: Chapter MUST include sensor integration in URDF (camera, lidar, IMU, force-torque) with appropriate Gazebo plugins for simulation (e.g., `libgazebo_ros_camera.so`)
- **FR-055**: Chapter MUST include brief instructor-led Gazebo demonstration (10-15 minutes) showing how student-created URDF models load in physics simulation to provide context for URDF learning and preview Module 2; students observe only without installing or using Gazebo themselves, with clear instructions on how instructors can prepare this demo.

#### Chapter 5: Launch Files and Package Management

- **FR-056**: Chapter MUST explain purpose of launch files for managing multi-node systems and complex startup configurations
- **FR-057**: Chapter MUST teach Python launch file syntax using `launch.LaunchDescription` and `launch_ros.actions.Node`
- **FR-058**: Chapter MUST demonstrate adding nodes to launch files with `Node()` action, including executable name, package, and node name
- **FR-059**: Chapter MUST show parameter passing to nodes in launch files using `Parameter` and `SetParameter` actions
- **FR-060**: Chapter MUST explain topic/service/action remapping in launch files using `Remap` action
- **FR-061**: Chapter MUST teach launch file arguments (`DeclareLaunchArgument`, `LaunchConfiguration`) for runtime configuration and conditional launching
- **FR-062**: Chapter MUST explain ROS 2 package structure (src, include, launch, config, urdf, test directories) and best practices for organization
- **FR-063**: Chapter MUST teach `package.xml` configuration (package metadata, build/exec dependencies, maintainer info) and common fields
- **FR-064**: Chapter MUST teach `setup.py` configuration for Python packages (entry points for nodes, data files) and `setup.cfg` for additional metadata
- **FR-065**: Chapter MUST demonstrate `colcon build` system for compiling packages and common build flags (`--packages-select`, `--symlink-install`)
- **FR-066**: Chapter MUST explain workspace setup and sourcing (`setup.bash`, overlay/underlay workspaces) and environment variables (`ROS_DOMAIN_ID`)
- **FR-067**: Chapter MUST teach dependency management with `rosdep` and how to resolve missing system dependencies
- **FR-068**: Chapter MUST provide complete working package example with multiple nodes, launch files, configuration files, and basic unit tests (`pytest`, `ament_cmake_pytest`)
- **FR-069**: Chapter MUST explain best practices for package organization, documentation (`README.md`), and versioning following semantic versioning principles

### Key Entities

- **ROS 2 Node**: Independent computational process with unique name in ROS graph; can publish/subscribe to topics, provide/call services, expose parameters, and interact with actions.
- **Topic**: Named communication channel for asynchronous message passing; supports many-to-many communication pattern; messages are strongly-typed.
- **Message**: Strongly-typed data structure passed between nodes over topics (e.g., `std_msgs/String`, `geometry_msgs/Twist`, `sensor_msgs/Image`).
- **Service**: Synchronous request-response communication mechanism with defined request/response message types; used for immediate, short-duration tasks.
- **Action**: Long-running task mechanism with goal, feedback, and result messages; builds on service pattern; used for complex, asynchronous behaviors.
- **Publisher**: Component of a node that sends messages to a topic; configured with QoS policies.
- **Subscriber**: Component of a node that receives messages from a topic via callback function; configured with QoS policies.
- **QoS Policy**: Configuration for communication reliability, durability, history depth, deadline, lifespan, and liveliness constraints; critical for real-time performance.
- **Parameter**: Runtime configuration value for nodes (string, integer, float, boolean, array types); allows flexible node behavior without recompilation.
- **Package**: Organizational unit containing nodes, launch files, configuration, and resources; follows specific directory structure.
- **Launch File**: Python script describing how to start and configure multiple nodes as a system; supports arguments, remapping, and conditional logic.
- **URDF Model**: XML description of robot structure including links (bodies), joints (connections), geometry, and physical properties; used for visualization and simulation.
- **Link**: Rigid body component of robot with visual appearance, collision geometry, and inertial properties; connected by joints.
- **Joint**: Connection between two links defining motion constraints (revolute, prismatic, fixed, continuous types); defines degrees of freedom.
...
- Students will have access to edge compute devices (Jetson) during Module 1, which will provide hands-on experience and enhance learning motivation.
...
- Standard ROS 2 Python client library (rclpy) and message packages (std_msgs, geometry_msgs, sensor_msgs, rcl_interfaces, tf2_msgs)
